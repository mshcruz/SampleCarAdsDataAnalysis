...
/**
 * Verify whether the word is a stop word and keep only its stem.
...
function processWord(word) {
  ...
  if (isStopWord(word) || /^\d+$/.test(word) || word.length < 3) {
    return '';
  }
  return stemmer(word);
}
...
/**
 * Verifies whether the specified word is a stop word.
 * @param {string} word The word to be verified.
 * @returns {boolean} Whether the specified word is a stop word.
 */
function isStopWord(word) {
  ...
  return stopwords.includes(word.trim());
}
